// Minimal WASI preview1 shim implementing only the ~23 functions ripgrep imports.
  ...
  const imports = {
    ...
    fd_write(fd, iovsPtr, iovsLen, nwrittenPtr) {
      ...
      try {
        ...
        for (let i = 0; i < iovsLen; i++) {
          ...
          if (e.type === "stdio") {
            if (e.which === 1) (stdout || process.stdout).write(Buffer.from(chunk));
            else if (e.which === 2) (stderr || process.stderr).write(Buffer.from(chunk));
            else return E.BADF;
            total += bufLen;
          } else if (e.type === "file") {
    ...
    fd_seek(fd, offset, whence, newOffsetPtr) {
      ...
      try {
        ...
        if (newPos < 0n) return E.INVAL;
        e.pos = newPos;
        dv().setBigUint64(newOffsetPtr, e.pos, true);
        return E.SUCCESS;
      } catch (err) {
        return errno(err);
      }
